...
class TypingApp(tk.Tk):
    ...
    def _on_complete_timed(self, timeout=False):
        ...
        # store last for SmartAnalysisDialog (before any nav)
        self._last_typed=typed_text
        self._last_target=target
        smart_hint=""
        smart_detail=""
        # ensure hint defined even if smart fails
        if SMART_AVAILABLE and analyze_text:
            try:
                ...
                # build hint to embed in grade dialog so para test ALWAYS shows what to practice
        ...
        # Always offer Smart Analysis FIRST — this is the fix: para test now shows mistakes/letters/drill
        opened_smart=False
        if SMART_AVAILABLE and getattr(self,'last_smart_report',None) is not None:
            if _mb.askyesno("Time Attack Completed — Smart Analysis", msg):
                try:
                    # use stored last target/typed so dialog shows correct para, not next exercise
                    SmartAnalysisDialog(self, self._last_target, self._last_typed, elapsed)
                    opened_smart=True
                except Exception as e:
                    print("smart dialog error", e)
            # after Smart dialog, ask Retry (separate, so user sees both)
            if _mb.askyesno("Retry?", "Retry same Time Attack (new para)?"):
                ...
                self._load_exercise()
                return
        # fallback if smart not available or user said no to smart
    ...
    def _on_complete(self):
        ...
        # Smart analysis capture (isolated, does not disturb timed para) — store before nav so dialog shows correct exercise
        typed_text=""
        try:
            typed_text=self.input.get("1.0","end-1c")
        except: pass
        target_text=self.current_text
        # keep for drill / last view
        self._last_typed=typed_text
        ...
        # Build smart hint to show inline with grade
        smart_hint=""
        smart_detail=""
        try:
            ...
            pass
        # Show completion dialog WITH smart hint (so para test always shows what to practice)
        ...
        # Step 1: Always offer detailed Smart Analysis (this is the fix — para test now shows mistakes/letters/drill)
        if SMART_AVAILABLE and getattr(self,'last_smart_report',None) is not None:
            ...
            # fallback if smart not available: show plain completion
            messagebox.showinfo("Completed", base_msg)
        # Step 2: Next exercise? (separate, so user always sees smart before deciding)
    ...
    def _show_smart_analysis(self):
        ...
        if not SMART_AVAILABLE or not analyze_text:
            ...
            return
        # Prefer last completed (para or normal) if current input is empty/short (user navigated)
        target = getattr(self, 'current_text', '')
        typed = self.input.get("1.0","end-1c") if hasattr(self, 'input') else ""
        elapsed = time.time()-self.start_time if self.start_time else 0
        # If we have last completed data and current typed is very short or analysis would be empty, use last
        has_last = hasattr(self, '_last_target') and hasattr(self, '_last_typed') and self._last_target and self._last_typed
        if has_last:
            # if current typed is empty or much shorter than last, or last report exists and current not yet typed
            cur_len = len(typed.strip())
            last_len = len(self._last_typed.strip())
            if cur_len < 5 or (last_len > cur_len + 10):
                target = self._last_target
                typed = self._last_typed
                # for elapsed, try to use time from last report if available
                try:
                    # keep elapsed as is; dialog will recompute anyway
                    pass
                except: pass
        if not typed or len(typed.strip()) < 2:
            # still not enough data but we have last report -> use it directly
            if has_last:
                target = self._last_target
                typed = self._last_typed
            else:
                messagebox.showinfo("Smart Analysis", "Type something first — then open Smart Analysis to see weakest letters & why (or complete an exercise).")
                return
        try:
